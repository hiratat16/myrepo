...
/*
問題
・68行目を効率良く動作するように書き換えるにはどうしたら良いか?
・free()が不要ななのは何故か?


・ 68行目: for文でリストを最初からチェックし, next が NULL である要素まで移動
          １行入力の度に, 最初から最後まで辿る事になり, 行数が増えたとき, 非常に非効率である.

・ free()は不要: プログラム終了時に malloc() で確保された領域や他の変数など,
                解放されるため,不要.
